...
% \toprule
% \textbf{section} & \textbf{entity} & \textbf{attributes} \\ \midrule
% \ref{sec:dis_neg} & \texttt{disorder} & negated \\
% \ref{sec:func_neg} & \texttt{function} & negated \\
...
Regarding scope, we annotate entities in form of noun or verb phrases together with their modifying parts, e.g.~adjectives, adverbs etc., though complex post modifiers (e.g., relative clauses: ``a pain that is \ldots"; and prepositional-phrase modifiers: ``a neck pain from yesterday") are excluded.
...
\examples{ex:scope}
    ...
    {(\ldots) ça fait \disorder{drolement mal} et \disorder{très genant}. \translation{It \disorder{hurts awfully}, and it is a \disorder{great discomfort} }} 
    ...
    {**\disorder{激しい嘔吐}に見舞われました \translation{I suffered \disorder{severe vomiting}.}} % I suffered severe vomiting.
...
\examples{ex:metaphors}
    ...
    {*\drug{マイスリー}を飲むと\anatomy{脳}が\disorder{強制終了}する \translation{\drug{Zolpidem} makes my \anatomy{brain} \disorder{shut down}}}
...
\examples{determiners}
    ...
    {**この\drug{薬}は\opNeg{効かなかった} \translation{This \drug{drug} \opNeg{didn't work}.}}
...
\examples{enumerations}
    ...
    {\measure{4} \other{opèrations},  \drug{himurel}, \drug{pentasa}, \drug{methotrexate}, (\ldots) \translation{\measure{4} \other{surgeries},  \drug{himurel}, \drug{pentasa}, \drug{methotrexate}, \ldots}}
...
% Example with ``lung'':
\begin{enumerate}
    \item ``it hurts in the \anatomy{left lung} and \anatomy{right lung}'' (two entities)
    
    \item ``pain on the left lung, but no issues were found on the right'' $\rightarrow$ annotation using two separate, partially overlapping entities, one continuous and one discontinuous as shown below:
    \begin{itemize}
        \item ``\disorder{pain} on the \anatomy{left lung}, but no issues were found on the right'' (continuous entity)
        \item \drawRelations
            {pain \& on the  \& left \anatomy{lung} \& , but \disorderNeg{no issues} were found on the \& \anatomy{right}}
            {\discontRel{3}{5}}\\
            (discontinuous entity) 
    \end{itemize}

    \item Similarly as above, the phrase ``nerves / muscle cramps'' is annotated as two separate, \textit{partially overlapping} entities:
    \begin{itemize}
        \item \drawRelations
    ...
    \end{itemize}
\end{enumerate}
\textcolor{red}{In case a discontinuous entity is necessary, we limit the number of entity fragments to two.}
...
\examples{ex:drug}
    ...
    {l'\drug{entyvio} c 'est pas miracle mais bon \opPos{ça aide}. bon courage à tous. \\ \translation{\drug{entyvio} is no miracle, but well, \opPos{it does help}. good luck to all.}}
...
\paragraph{\textcolor{red}{Prioritization of \dr}} \textcolor{red}{In case the \texttt{route} and \dr are one word (e.g., a compound) or connected by a hyphen, which might be the case for German, we prioritize \dr over \texttt{route} and label the entire spans as \dr.}
...
To specify what kind of change occurred, we add the attributes \texttt{start} (the trigger specifies that a medication was just started), \texttt{stop} (a medication was stopped), \texttt{increase} (the dosage was increased), \texttt{decrease} (the dosage was decreased), and \texttt{unique\_dose} (the medication was only given/taken once) \emph{to the \dr they refer to} (see below in Section \ref{sec:drug_change_attributes}).
...
\examples{ex:att_drug}
    ...
    {*\disorder{副作用}が強くて、\drugStop{レクサプロ}\trigger{辞めました}！ \translation{I \trigger{stopped} \drugStop{Lexapro} because a \disorder{side effect} was strong}} 

If the patient describes having forgotten the medicine, we add the attribute \texttt{decrease} except the context suggest a complete stop (then we add \texttt{stop}).
Conversely, if she describes taking, for example, a double dose, this is to be marked as an \texttt{increase}.
...
\examples{ex:drug_abbr}
    ...
    {(\ldots) je passe une \test{radio} des \anatomy{2 mains} \timexPIT{mercredi matin}, avant ma \drug{perf}, (...)\\ \translation{I'm having a \test{CT} for \anatomy{both hands} on \timexPIT{Wednesday morning}, before my \drug{IV}}}
...
\examples{ex:drug_ref}
    ...
    {Ich nehme die \drug{Tabletten} \timexDur{seit 2 Tagen}. \translation{I take those \drug{pill} \timexDur{for two days}.}}
...
\examples{ex:disorder}
    ...
    {J'ai une \disorder{maladie de crohn} \timexDur{depuis 36 ans}~ (\dots) \\ \translation{I've had \disorder{crohn's disease} \timexDur{for 36 years}~ (\dots)}}
...
\begin{enumerate}
    \item If the entity in question is a noun phrase and describes a malfunction, we label it \disorder{\dis} rather than \functionNeg{negated function}:
        ``I have \disorder{no appetite}.''
    \item If, however, a malfunction is expressed beyond a continuous noun phrase, we label it \functionNeg{negated function}:
...
For \dis, note that we do not give ``negated'' value to the disorders that do not completely disappear (e.g. ``a \disorder{long-running headache} was \opPos{almost eased}'') because they still exist at least a little.
...
\examples{negation}
    ...
    {Par contre j'étais pas plus \disorderNeg{fatigué} que la normale.\translation{however, I wasn't more \disorderNeg{tired} than usual.}}
    {Folgende Beschwerden sind schon weg: \disorderNeg{Nackenschmerzen}, \ldots \translation{The following ailments are already gone: \disorderNeg{neck pain}, }}
    {*\disorderNeg{イライラ}などはしなかった \translation{I didn't \disorderNeg{feel irritated} or something}} % I didn't feel irritated or something

\subsubsection{\textcolor{red}{Partial Negation (of Disorders)}}\label{sec:partial_negation}

\textcolor{red}{We \emph{do not} annotate partial negations since these require a more detailed annotation which is, for now, out of scope.}

\examples{ex:partial_negation}
    {My \anatomy{hair} was \emph{not lost entirely}.}
    {}
    {Die \anatomy{Brust} tut schon viel weniger weh. \translation{The \anatomy{chest} hurts much less already.}}
    {}
...
\examples{ex:function}
    {\drug{Lipidor} \opPos{did the job} on my \function{cholesterol} both LDL and HDL.}
    {La seule chose qu'on m'a donné pour essayer de \function{dormir} c'est du \drug{Xanax} \translation{\drug{Xanax} is the only thing I was given to help me \function{sleep}}}
    {\drugStop{Opripramol} \trigger{hatte} ich ja nur \timexDur{zwei Abende lang}~zum \function{Schlafen} je \measure{eine halbe} genommen, also nur eine \measure{winzige Dosis}. \translation{I \trigger{had} only taken \measure{half a dose} of \drugStop{Opripramol} \timexDur{for two evenings} for \function{sleeping}, so only a \measure{tiny dose}.}}
    {**\function{白血球}は問題なかった。\translation{no problem with \function{leukocyte}}} % no problem with leukocyte
...
\examples{ex:anatomy}
    ...
    {**\anatomy{お腹}が\disorder{刺すように痛い} \translation{I feel a \disorder{stabbing pain} in my \anatomy{stomach}}} % I feel a stabbing pain in my stomach (範囲は全体を対象とする：e.g. お腹から頭にかけて）
...
\examples{ex:anatomy_embedded}
    ...
    {**\disorder{肺がん}と宣告されてしまった \translation{I was diagnosed with \disorder{lung cancer}.}}
...
\examples{ex:test}
    ...
    {**\test{血液検査}の結果が怖いです \translation{I'm afraid of the \test{blood test} results.}} % I'm afraid of the blood test results.
...
This denotes the \textcolor{red}{writer's} \textit{personal} evaluation or opinion of a medication (\dr), health state (\dis) or biological process (\texttt{function}).
...
The opinion always comes with a sentiment attribute: either \texttt{positive}, \texttt{negative}, or \texttt{neutral}.
...
\textcolor{red}{We annotate \emph{all} opinions to make it easier for the annotators and keep annotation consistent with the other entities. 
...
\examples{ex:eval}
    ...
    % 
    {*This \drug{drug} is a \opNeg{nightmare} and \opNeg{should be discontinued}.}
    ...
    {*\drug{アモキサン}\opPos{効いてる}おかげか\function{気持ち}は\opPos{少し楽になった} \translation{My \function{feeling} becomes \opPos{a little better} now that the \drug{amoxan} \opPos{is working}.}} % I'm feeling a little better now that the amoxan is working.
    
\paragraph{\textcolor{red}{Emojis / Emoticons}} \textcolor{red}{We include emojis or emoticons if they explicitly encode an opinion. }
...
\examples{ex:measure}
    ...
    {\drug{pentasa}, \drug{cortancyl}  (\measure{10mg}), \ldots \translation{\drug{pentasa}, \drug{cortancyl}  (\measure{10mg}), \ldots}}
    {Das \drug{Utrogest} sind \route{Weichkapseln} mit \measure{100 mg} \drug{naturid.~Progesteron} und sie sind verschreibungspflichtig. \translation{\drug{Utrogest} are \route{soft capsules} with \measure{100 mg} of \drug{nature-id. progesterone} and they are prescription-only.}}
    {\test{生検}の結果\function{Ki67}が\measure{46％}だったため、通院にて治療中です \translation{The \test{biopsy} results showed a \function{Ki67} of \measure{46\%}, and I am in the hospital for treatment. }} % The biopsy results showed a Ki67 of 46\%, and I am in the hospital for treatment. 

In some cases it might happen that it is difficult to distinguish between a phrase being a \texttt{opinion} or a \texttt{measure} when they occur together with a \texttt{test} (see for example \ref{sec:diff_cases_en}, Ambiguity). 
In these cases, we prioritize \texttt{measure.}

\subsubsection*{Temporal Measurements}

Note that temporal measurements (e.g. ``5 times per month'') should be annotated as \texttt{time} entities (see Section \ref{ent_timex}) \textit{\textbf{unless}} they indicate the amount/dosage of a drug:
If we can relate the expression with the \texttt{dosage} relation to a \dr, then it is probably a \texttt{measure}.

\begin{enumerate}

\item **I took \measure{\textcolor{white}{three days' worth}} of \drug{medicines} at once \timexDate{today}~.

\item **Mom should take this \drug{pill} \measure{\textcolor{white}{three times a day}}.
\end{enumerate}
...
If none of the attributes fits, we do not add any.

\subsection{Route} \label{ent_route}

\texttt{Route} annotates the means of medication intake, e.g.~via pills or via injection.
...
\examples{ex:route}
    ...
    {\ldots, \timexDur{depuis 1 an et demi}, \route{perfusion} d \drug{entyvio} \timexFreq{tous les mois}. \ldots \translation{\timexDur{for the past 18 months}, I get \drug{entyvio} \route{IV}  \timexFreq{every month}.}}
    {Ich habe das leider nur mit \route{Tabletten} gemacht (mir wollte keiner meiner \doctor{Ärzte} eine \route{Infusion} verschreiben). \translation{Unfortunately, I only did this with \route{pills} (none of my \doctor{doctors} would prescribe me an \route{infusion}).}}
    {*現在\route{内服}しているのは\drug{レトロゾール}です \translation{I am currently taking \drug{letrozole} \route{orally}.}} 
...
This entity tag is for job descriptions \textit{not} for the names of the doctors.
...
\examples{ex:doctor}
    ...
    {( peut etre de \disorder{l'arthrose} dixit mon \doctor{docteur traitant}) \ldots \translation{(could be \disorder{arthritis} according to my \doctor{GP})}}
    ...
    {**病院の\doctor{先生}がこの薬を\route{飲め}って言うんです。でも信用できません \translation{The \doctor{doctor} at the hospital says I should \route{drink} this medicine. But I don't trust him.}} 


\subsection{Entities for De-Identification}\label{sec:deident}

To make sure personal information like user names, URLs, e-mail addresses, etc. do not end up in the final corpus, even if we pre-processed the data, we also mark these information with the respective labels to later apply a final post-processing.
...
\examples{ex:user}
    ...
    {Bonjour \user{Mûre Vive} pour ma part ils \opNeg{n'ont pas été efficace} (\ldots) \translation{Hi \user{Mûre Vive}, for me they were \opNeg{not effective}}}
    {Liebe \user{Broccoli Klarinette} danke für deinen Bericht. \translation{Dear \user{Broccoli Klarinette}, thanks for your report.}}
    {\user{ゆーりん}さん、ありがとうございます！\translation{Thank you, \user{Yurin}!}} 
...
\examples{ex:url}
    ...
    {Si vous souhaitez discuter plus avant: \link{www.xxx xxxx xxx xxx}. \translation{If you wish to discuss further: \link{www.xxx xxxx xxx xxx}.}}
    {Wenn du möchtest, geh doch mal auf die Seite \link{www.xxx xxxx xxx xxx}. \translation{If you like, take a look at this website: \link{www.xxx xxxx xxx xxx}.}}
    {ここに詳しく載ってます \link{www．ｘｘｘ．ｘｘ}} 
...
\examples{ex:info}
    ...
    {J'ai appris que j'avais \disorder{crohn} \timexPIT{il y a cinq ans}. J’avais \info{20 ans}. \translation{I was diagnosed with \disorder{crohn} \timexPIT{five years ago}. I was \info{20 years old}.}}
    {*Ich wohne in \info{Musterhausen}, das kennst du sicher. \translation{I live in \info{Musterhausen}, you probably know it.}}
    {**\info{伊藤晴美}、\info{大学2年生}です。\translation{I am 
...
\examples{ex:other}
    ...
    {}
    {Ohne BH fallen die \other{Implantate} runter. \translation{Without bra the \other{implants} fall off}.}
    {*\drug{抗がん剤}の\disorder{副作用}で\anatomy{髪}が\disorder{抜け}て、\other{ウィッグ}を着用していました。\translation{Wore a \other{wig} because of \disorder{hair loss} due to \disorder{side effects} of \drug{anti-cancer drugs}}} % Wore a wig because of hair loss due to side effects of anti-cancer drugs

\textcolor{red}{what about OPs? e.g. aesthetic surgery?}

\textcolor{red}{partial negation \& entailment are future work; see difficult cases section}
...
\examples{ex:caused}
    ...
    {\drawRelations{
        ...
        \drug{レクサプロ} \& 飲み始めてから \& \disorder{吐き気} \& はするし \& \disorder{食欲無い} \& し 
        }{
        \causedRel{1}{3}
        % \changeRel{2}{1}
        \causedRel{1}{5}
...
\subsubsection{\textcolor{red}{Special Case: Effects of Stopping Medication Intake}}

\textcolor{red}{Officially, drug withdrawal is an adverse drug reaction. Therefore, we include it in the annotations and draw a \texttt{caused} relation between the \dr and the withdrawal symptoms (\dis).}

\textcolor{red}{Stopping a medication might, however, also bring back the original symptoms or disorder. To ease the annotation burden and make the annotation consistent, we always draw a \texttt{caused} relation between a \textit{stopped} \dr and the resulting \dis. Note that for the latter, there is most probably also a \texttt{treatment\_for} relation.}
...
%drawRelations{
%        I have \& \disorder{insomnia} \& and \& fibromyalgia. \\My doctor prescribed this \& \drug{medication} \& to me mainly for better sleep.
%        }{
%        \treatmentRel{5}{2}
%        }
%    }
...
\examples{ex:dosage}
    ...
    {} 
    % {\drawRelations{\timeDur{depuis 1 an et demi} \& ,perfusions d \& \drug{entyvio} \& tous les mois. \& \drug{pentasa}\& , \& \drug{cortancyl} \&  (\& \measure{10mg}), , \ldots }{\dosageRel{4}{5}}}
...
\examples{ex:experienced}
    ...
    {\drawRelations{**Ich hatte einen \& \disorder{seltsamen Schmerz} \& im \& \anatomy{Bein} \& .}{\experiencedRel{2}{4}}}
    {\drawRelations{
...
\texttt{anatomy} entities can be examined with a test, too.

\examples{ex:examined}
    {}
    {\drawRelations{je passe une \& \test{radio} \& des 2 \& \anatomy{mains} \& mercredi matin, \ldots}{
        \examinedRel{2}{4}
        }}
    {\drawRelations{
        **Meine \& \disorder{Schmerzen} \& wurden dann mit einer \& \test{Magenspiegelung} \& untersucht.
        }{
        \examinedRel{2}{4}
        }
    }
...
Following the line of thought from above, the \texttt{has\_result} relation then relates the \texttt{test} with its result, usually a \texttt{measure}.
...
\examples{ex:resulted}
    ...
    {}
    {\drawRelations{
        **Die \& \test{Spiegelung} \& zeigte \& \measure{keinerlei Ergebnisse}.
        }{
        \resultedRel{2}{4}
        }
    }
    {\drawRelations{
        **先月 \& の \& \test{検査} \& では \& 白血球 \& は \& \measure{5000} \& でした
        }{
        % \timeRel{1}{3}
        % \examinedRel{3}{5}
        \resultedRel{3}{7}
...
It is used to associate similar concepts, but \emph{not necessarily} to connect the same surface forms (e.g.~``Lipitor'' and ``Lipitor''). 
Via the \texttt{refers\_to} relation, we can automatically propagate the relation to all connected targets.
The relation allows to chain corresponding entities.
...
\examples{ex:refers}
    ...
    {}
    {\drawRelations{
        **Ich hatte \& \disorder{Schüttelfrost} \& und \& \disorder{zitterte} \& am ganzen Körper.
        }{
        \refersRel{2}{4}
        }
    }
...
\examples{ex:interacted}
    ...
    {}
    {\drawRelations{
        **Die Kombination aus \& \drug{Grapefruitsaft} \& und \& \drug{Aspirin} \& hat meine Schmerzen nur verschlimmert.
        }{
        \interactedRel{2}{4}
        }
    }
...
\examples{ex:re_change}
    ...
    {\drawRelations{
        **Ich habe mit \& \drugStart{Triseqenz Hormontabl.} \& \trigger{angefangen} \& und fühle mich super.
        }{
        \changeRel{2}{3}
        }
    }
...
\examples{ex:re_time}
    ...
    {\timeRel{3}{2}}
     ...
     {\drawRelations{
        \timexDur{**Seit einer Woche} \& nehme ich \& \drugStart{Triseqenz Hormontabl.} \&  und fühle mich super.
        }{
        \timeRel{2}{1}
        }
    }
...
The \texttt{has\_route} relation associates a drug with the way it is consumed, i.e.~the means of intake.

\examples{ex:means}
    {}
    {}
     {\drawRelations{**Ich hatte gestern leider vergessen mit das  \& \drug{Insulin} \& zu \& \route{spritzen} \& .}{\meansRel{2}{4}}}
    {\drawRelations{%
...
Mostly, these will be \texttt{drug} or \texttt{function} entities.
Note that sometimes it is not possible to find two explicit entities to be connected. 

\examples{ex:re_eval}
    {}
    {}
    {\drawRelations{**Seit ich  \& \drugStart{Triseqenz Hormontabl.} \& nehme, fühle ich mich \& \opPos{super} \& .}{\evalRel{4}{2}}}
    {\drawRelations{%
...
If we clearly see a worthwhile relation between two entities in terms of adverse drug events, but there is no relation defined (yet), we can use the \texttt{misc} link.
...
\examples{ex:entailment1}
    {My eyebrows remain halfway. \textit{(meaning the other half is gone)}}
    {}
    {}
    {}
...
\subsection{\textcolor{red}{Advice from other Users}}

\textcolor{red}{In some cases, one user is repeating the symptoms of another user in their own text and gives some advice. Since we are annotating the data document-wise, we do not take the original patient's view into account in this case. Therefore, we only annotate the entities, but not the relations.}

\examples{advice}
    {**\emph{Your} \disorder{fatigue} is certainly due to the \disorder{heavy bleeding}.}
    {}
    {}
    {}
...
\begin{description}
    ...
    \item[Ambiguity]
    \test{EKG} \measure{perfect}, \test{tri's} were \measure{perfect}, \function{blood sugar} \measure{perfect}, etc \\ Is \textit{blood sugar} a \texttt{function} or a \texttt{test}? Following up on this, if \textit{perfect} an \texttt{opinion} or a \texttt{measure}?
    
    \item[...] {\drug{Lipidor} \opPos{did the job} on my \function{cholesterol} both LDL and HDL.}\\
    Should \textit{LDL} and \textit{HDL} be annotated as \texttt{function}?
\end{description}
