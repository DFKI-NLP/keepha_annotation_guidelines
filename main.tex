...
The data for this corpus consists of different sources, e.g., patient foram, social media like Twitter, \textcolor{red}{TBD}.
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \drawRelations
            {\disorder{pain} \& on the left \& \anatomy{lung} \& , but no issues were found on the \& \anatomy{right} \& (discontinuous) }
            {\discontRel{3}{5}}
    \end{itemize}
...
This denotes the patient's personal evaluation or assessment of a (\dr), health state (\dis) or biological process (\texttt{function}).
...
With \texttt{measure}, we mark all kinds of measurements, e.g.~drug dosages or test results.
...
\ref{sec:sent}& \texttt{evaluation / assessment} & positive; negative; neutral\\
...
\texttt{Other} entities can be arguments of all relations below.
...
\begin{itemize}
    ...
    \item The cause--consequence is not explicitly attributed to the author of the text, nor to a particular person or group of people identified via co-text or context (including the meta data of the document).
\end{itemize}
