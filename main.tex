...
\examples{discontinuous}
    ...
    {\drawRelations{\ldots obgleich meine \& \disorder{Progesteronwerte} \& seit Jahren \& \disorder{im Keller} \& waren.}{\discontRel{2}{4}} \translation{\ldots although my \disorder{progesterone values} have been in the \disorder{lowest levels} for years.}}
    {...}
...
\examples{ex:partial_negation}
    {\translated My \anatomy{hair} was \emph{not lost entirely}.}
...
\noindent \texttt{Anatomy} entities are \emph{not annotated} when within a larger entity such as within a \dis or \texttt{test}.
...
This denotes the \emph{personal} evaluation or opinion of a medication (\dr), health state (\dis) or biological process (\texttt{function}).
...
We annotate \emph{all} opinions to make it easier for the annotators and keep the annotation consistent with the other entities. 
...
The relation time connects a \texttt{time} expression with anything except other \texttt{time} expressions.
...
\examples{ex:re_eval}
    {\drawRelations{*I started taking the \& \drug{mirtazapine} \& again, and I \& \opPos{felt much better}.}{\evalRel{4}{2}}}
...
If we clearly see a worthwhile medical relation between two entities in terms of adverse drug events, but there is no relation defined (yet), we can use the \texttt{misc} link.

\examples{ex:misc}
    {\drawRelations{*(\ldots) in between a \& \other{cardioversion} \& in mid-May between the \& \other{ablations}.}{
        \miscRel{2}{4}
    }}
...
\textcolor{red}{In some cases, one user repeats the symptoms of another user in their own text and gives some advice. Since we are annotating the data document-wise, we do not take the original patient's view into account in this case. Therefore, we only annotate the entities, but not the relations.}

\examples{advice}
    {**\emph{Your} \disorder{fatigue} is certainly due to the \disorder{heavy bleeding}.}
    {}
    {**Gegen die \disorder{Hitzewellen} \emph{würde ich} \drug{Johanniskraut} nehmen.}
    {}
...
\begin{description}
    \item[Semantics] \drawRelations{
        ...
        $\rightarrow$ right \emph{lung} or right side of the entire body?
        
    \item[Discontinuity]
        \drawRelations{*I took \& \measure{{three days' worth}} \& of \drugInc{medicines} \& \measure{at once} \& \timexDate{today}.}{
        \discontRel{2}{4}
        }\\
        $\rightarrow$ discontinuous entities in general are often difficult
    
    \item[Ambiguity]
    ``\test{EKG} \measure{perfect}, \test{tri's} were \measure{perfect}, \function{blood sugar} \measure{perfect}, etc.'' \\ $\rightarrow$  Is \textit{blood sugar} a \texttt{function} or a \texttt{test}? Following up on this, if \textit{perfect} an \texttt{opinion} or a \texttt{measure}?
    
    \item[Specific manifestations of the same function] {\drug{Lipidor} \opPos{did the job} on my \function{cholesterol} both \emph{LDL} and \emph{HDL}.}\\
