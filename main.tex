...
\examples{ex:metaphors}
    {I felt always like I had \disorder{a veil over my head} and \disorder{lead in my legs}.}
    {(\ldots) à chaque effort, mon \anatomy{cœur} \disorder{bat la chamade} et jai ne n'ai pas \disorder{beaucoup de souffle}.}
...
\examples{determiners}
    ...
    {(\ldots) J’ai développé une très forte \disorder{sinusite chronique},(\ldots)}
...
However, we \textit{do not} annotate other treatments, for instance, ``chemo therapy''.
...
\examples{ex:anatomy}
    ...
    {\ldots, des \disorder{boutons} de la \anatomy{tête} au \anatomy{pied}, \ldots}
...
\examples{ex:measure}
    {}
    {\ldots, bon ça fait beaucoup, depuis 1 an et demi ,perfusions d entyvio tous les mois. \drug{pentasa}, \drug{cortancyl}  (\measure{10mg}), \ldots}
...
\examples{ex:time}
    {Instead of taking the \drugDec{pill} \timexDur{2X daily}, as prescribed I take it (\ldots) usually about \timexDur{3-4 times per week}, but often skipping a week or two at a time.}
...
\examples{ex:route}
    ...
    {\ldots, depuis 1 an et demi, \route{perfusion} d \drug{entyvio} tous les mois. \ldots}
...
\examples{ex:user}
    {*Dear \user{XXX}, thank you for your message.}
    {Bonjour \user{XXX} pour ma part ils n'ont pas été efficace (\ldots)}
    {Liebe \user{XXX} danke für deinen Bericht.}
    {} 
...
\examples{ex:info}
    {**My name is \info{XXX XXX} and I am \info{XX years old}.}
    {}
    {Ich wohne in \info{xxxx}, das kennst du sicher.}
    {} 
...
\texttt{frequency}, \texttt{duration}, \texttt{date} and \texttt{relative point in time}. The attributes should help to specify the annotated timex expressions in more detail. Some examples below:
