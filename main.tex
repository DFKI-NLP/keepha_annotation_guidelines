...
\begin{enumerate}
    ...
    \end{itemize}
\end{enumerate}
\textcolor{red}{In case a discontinuous entity is necessary, we limit the number of entity fragments to two to reduce the burden for the annotators.}

\subsubsection*{Punctuation}

Punctuation is not to be included in the annotation (except for, e.g., hyphens, abbreviations, etc.).
...
\examples{ex:partial_negation}
    ...
    {**Les \disorder{attaques de panique} ont \emph{presque disparu}.
...
\textcolor{red}{We annotate \emph{all} opinions to make it easier for the annotators and keep the annotation consistent with the other entities. 
...
The attributes should help to specify the annotated time expressions in more detail. See examples in Section \ref{ent_timex}.
...
We annotate all medical job descriptions (e.g., physician, dentist, nurse, (psycho) therapist \ldots).
...
\examples{ex:other}
    ...
    {*\drug{抗がん剤}の\disorder{副作用}で\anatomy{髪}が\disorder{抜け}て、\other{ウィッグ}を着用していました。
...
\examples{ex:caused}
    ...
    {\drawRelations{
        Elle a pris \& de l'\drugStop{infliximab} \& mais (\ldots) elle est devenue \& \disorder{toute rouge} \& et \& \disorder{nauséeuse}}
        {
        \causedRel{2}{4} 
        \causedRel{2}{6}
        }
...
\examples{ex:examined}
    {*\drawRelations{I'm having \& \anatomy{both hands} \& \test{x-rayed} \& \timexPIT{on Wednesday morning}~, \ldots}{
        \examinedRel{2}{3}
        }
    }
...
\examples{ex:refers}
    {*\drawRelations{I had \& \disorder{chills} \& and my \anatomy{whole body} was \& \disorder{shaking}}{
        \refersRel{4}{2}
    }}
...
\examples{ex:interacted}
    {\drawRelations{I've already tried \& \drug{Laiff} \& \measure{900}, it reduces the effect of my \& \drug{progestérone}.}{
        \interactedRel{2}{4}
    }}
    ...
    {\drawRelations{
        **Die Kombi aus \& \drug{Grapefruitsaft} \& und \& \drug{Aspirin} \& hat meine \disorder{Schmerzen} nur verschlimmert.
        }{
        \interactedRel{2}{4}
        }
    }
...
\examples{ex:re_change}
    {\drawRelations{**Last week, I \& \trigger{started} \& the \& \drug{medication therapy}. }{
        \changeRel{2}{4}
    }}
    {\drawRelations{
...
\examples{ex:means}
    {\drawRelations{*I have to say right away that I cannot tolerate \& \drug{progesterone} \& \route{capsules} \& at all.”}{
        \meansRel{2}{3}
    }}
    {\drawRelations{Je dois dire tout de suite que je ne tolère pas du tout les \& \route{capsules} \& de \& \drug{progestérone}.}{
        \meansRel{2}{4}
    }}
     {\drawRelations{**Ich hatte gestern leider vergessen mir das  \& \drug{Insulin} \& zu \& \route{spritzen} \& .}{\meansRel{2}{4}}}
    {\drawRelations{%
...
\examples{ex:re_eval}
    {}
    {\drawRelations{J'ai pris (\ldots) la mirtazapine. L'année dernière, j'ai recommencé à en prendre, et je me suis sentie beaucoup mieux.}{}}
